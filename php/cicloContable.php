<?php
	...
	*/
	class cicloContable extends model{
		//private $table
		...
		public function showTable($table){
			...
			$result = $this->_db->query("SELECT * FROM empresa WHERE  show_by= '1'");
			$retorna = $result->fetch_all(MYSQL_ASSOC);
			...
			foreach ($retorna as $clave => $valor) {
			    ...
			    $outp_select[] = array('id'=> $valor['idEmpresa'],
									   'value'=> $valor['nom_empresa']
										);
			...
			*/
			...
			// Creacion de campos
			$outp[] = array('name'=> "Empresa_idEmpresa",
							'label'=> "EMPRESA",
							'tipo'=> "select",
							'class'=> "",
		...
		public function showTableAllModify($table, $nom_idTable, $id){
			...
			//tabla select all
			$result_select = $this->_db->query("SELECT * FROM empresa WHERE show_by = '1'");
			...
			$result_selectId = $this->_db->query("SELECT * FROM empresa WHERE show_by = '1' and idEmpresa = '".$retorna_main[0]['Empresa_idEmpresa']."'");
			...
			foreach ($retorna_selectId as $clave => $valor) {
    			//print_r($valor['categoria_producto']);
			    $outp_selectId = array('id'=> $valor['idEmpresa'],
									 	 'value'=> $valor['nom_empresa']
									    );
			...
			*/
			...
			$outp[] = array('name'=> "obs_ccontable",
							...
							'tipo'=> "text",
							'class'=> "",
							'value'=> "".$retorna_main[0]['obs_ccontable']."");
	...
	//objeto a manipular
	$object = new cicloContable();
	$table = "cicloContable"; //nombre de la tabla
	$nom_idTable = "idCicloContable";
