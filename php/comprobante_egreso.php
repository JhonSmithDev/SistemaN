	...
	*/
	class comprobanteEgreso extends model{
		...
		public function getComprobante($_idCicloContable){
			$result = $this->_db->query("SELECT * FROM egreso eg, comprobante com , librodiario lb, ciclocontable cc WHERE eg.Comprobante_idComprobante = com.idComprobante and com.idComprobante = lb.Comprobante_idComprobante and lb.CicloContable_idCicloContable = cc.idCicloContable and cc.show_by = 1 and cc.idCicloContable = '".$_idCicloContable."'");
			$retorna = $result->fetch_all(MYSQL_ASSOC);
